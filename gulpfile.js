...
// js task
function jsTask() {
  return src(files.jsPath).pipe(dest("dist/assets/js"));
}
...
function scssMove() {
  return src(files.scssPath).pipe(dest("dist/assets/sass"));
}

function sendMailMove() {
  return src("app/mail/**/*").pipe(dest("dist/mail"));
}
...
//Image Task
function imageTask() {
  return src(files.imagePath).pipe(gulp.dest("dist/assets/images"));
}
...
function watchTask() {
  watch(
    [files.scssPath, files.jsPath, files.imagePath, files.njkPath],
...
exports.combine = series(combineTask, reload);
